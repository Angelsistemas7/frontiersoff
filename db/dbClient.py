...
class DbClient(withMetaclass(Singleton)):
    ...
    def parseDbConn(cls, db_conn):
        ...
        # "rediss://" (TLS) se trata como REDIS normal + flag ssl aparte,
        # lo exigen proveedores gratis como Upstash.
        cls.db_ssl = isTlsScheme(scheme)
        cls.db_type = "REDIS" if scheme == "REDISS" else scheme
    ...
    def __initDbClient(self):
        ...
        kwargs = dict(host=self.db_host, port=self.db_port, username=self.db_user,
                      password=self.db_pwd, db=self.db_name)
        if self.db_type == "REDIS":
            kwargs["ssl"] = self.db_ssl
        self.client = getattr(__import__(__type), "%sClient" % self.db_type.title())(**kwargs)
